...
*** 条件语句字典化
    字典构建了从词到值的映射关系，当条件语句有这样的特点时，可用字典方便地替代。体会下面的例子，已知学生群体的变量名 =aff= ，找出学生人数：
...
*** Python 内部的字典
    字典是 Python 的核心数据结构，它的命名空间（namespace）就是用字典实现的。Python 环境中的变量都是某个字典中的词。往往字典的妙用可以给程序带来神来之笔的重构。
